...
class InertialExtendedKalmanFilter15D:
    """Classical inertial error-state EKF.

    Nominal state:
        p, v, R, gyro_bias, accel_bias

    Error-state order:
        [dtheta, dv, dp, dbg, dba]

    The input convention intentionally matches the 15D InEKF wrappers:
        control = [ax, ay, az, gx, gy, gz]
    where acceleration can be body-frame specific force or world-frame
    acceleration depending on ``translation_input_frame``.
    """
    ...
    def __init__(
        ...
        if pose_type == "6d":
            pose_type = "3d"
        if pose_type != "3d":
            raise ValueError("InertialExtendedKalmanFilter15D supports only 3d pose.")
    ...
    @classmethod
    def from_configs(cls, dataset_config: dict, compare_config: dict) -> "InertialExtendedKalmanFilter15D":
        cfg = compare_config.get("inertial_ekf_15d", compare_config.get("inertial_extended_kalman_filter_15d", compare_config))
        return cls(
    ...
    def predict(self, control: Iterable[float] | None, dt: float) -> np.ndarray:
        ...
        if u.size < 6:
            raise ValueError("3D inertial EKF control must contain [ax, ay, az, gx, gy, gz].")
        ...
        F = self._continuous_error_jacobian(R_prev, w, a)
        self.Phi = _matrix_exp(F * dt)
        self.Q = diagonal_covariance(self.process_noise_diag)
    ...
    def _continuous_error_jacobian(self, R: np.ndarray, w: np.ndarray, a: np.ndarray) -> np.ndarray:
        F = np.zeros((self.error_dim, self.error_dim), dtype=float)
        F[0:3, 0:3] = -lie.skew(w)
        F[0:3, 9:12] = -np.eye(3)
        F[6:9, 3:6] = np.eye(3)

        if self.translation_input_type == "acceleration":
            if self.translation_input_frame == "body":
                F[3:6, 0:3] = -R @ lie.skew(a)
                F[3:6, 12:15] = -R
            elif self.translation_input_frame == "world":
                F[3:6, 12:15] = -np.eye(3)
        return F

    def _inject_error(self, delta: np.ndarray) -> None:
        delta = np.asarray(delta, dtype=float).reshape(self.error_dim)
        self.Rot = self.Rot @ lie.so3_exp(delta[0:3])
